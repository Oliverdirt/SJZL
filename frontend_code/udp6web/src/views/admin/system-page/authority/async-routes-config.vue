<template>
  <div>
      ...
      <span class="custom-tree-node" slot-scope="{ node, data }">
        ...
        <span>
          <template v-if="data.id === 0">
            <Button class="add-root-btn" type="primary" size="small" icon="ios-add"
                    @click="editNode(node, data, 'append')">新增</Button>
          </template>
          <template v-else>
            <Button type="default" icon="ios-add"  size="small" @click="editNode(node, data, 'append')">新增</Button>
            <Button type="default" icon="md-create" size="small" @click="editNode(node, data, 'update')">编辑</Button>
            <Button type="default" icon="ios-remove" size="small" @click="removeNodeConfirmed(node, data)">删除</Button>
    ...
    <div class="modal-mask" v-if="isNodeEdited">
      <div class="modal-mask scroll-box">
        <div class="modal-mask-content">
          <Card class="modal-mask-form">
            ...
            <Form ref="asyncRoute" :model="asyncRoute" :rules="ruleAsyncRoute">
              ...
              <FormItem prop="title" label="标题（title）">
                <Input type="text" v-model.trim="asyncRoute.title" placeholder="请填写路由标题">
              ...
              <FormItem prop="roleId"  v-show="!$util.sassModel" :label="'角色：'">
                <Select ref="roleSelect" v-model="asyncRoute.roleId" multiple :disabled="asyncRoute.flag === 0 ? true : false">
              ...
              <Row type="flex" justify="center">
                <Button type="primary" @click="handleSubmit('asyncRoute')" :disabled="isSubmitDisabled">保存</Button>
    ...
    return {
      ...
      ruleAsyncRoute: {
        title: [{ required: true, message: '请填写路由标题', trigger: 'blur' },
          { max: 30, message: '标题不可以超过30个字', trigger: 'blur' }],
        name: [
          {
            required: true,
            message: '请填写路由名',
    ...
    handleSubmit (name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          //   this.asyncRoute
          // ]
          addOrUpdateMenuByData(this.asyncRoute)
          // this.$http.put(url, data)
            .then(response => {
              if (isAppendNode) {
                this.asyncRoute.id = response.data.id
