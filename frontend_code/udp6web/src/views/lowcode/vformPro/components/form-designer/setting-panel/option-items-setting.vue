<template>
  <div class="option-items-pane">
    ...
    <div style="margin-bottom: 8px;"
         v-if="((selectedWidget.type === 'checkbox')||(selectedWidget.type === 'select')||(selectedWidget.type === 'radio'))">
      <label class="el-form-item__label" style="width: 120px;">绑定数据字典 </label>
      <el-switch
        v-model="optionModel.isDic"
        @change="switchChange">
      </el-switch>
    </div>
    <div v-if="(optionModel.isDic)" style="margin-bottom: 8px;">
      <label class="el-form-item__label">数据字典</label>
      <el-select v-model="dicSelect" placeholder="请选择" @change="change">
        <el-option
          v-for="item in dicArr"
          :key="item.dicCode"
          :label="item.dicName"
          :value="item.dicCode"
        >
...
<script>
  ...
  export default {
    ...
    methods: {
      switchChange (data) {
        this.optionModel.isDic = data
      },
      change (data) {
        // alert(data)
        var objVal = {}
        this.dicArr.forEach((val) => {
          if (val.dicCode === data) {
            objVal = val
            this.optionModel.dicCode = val
          }
        })
        this.dicSelect = objVal.dicName

        // 获取字典项 /cscpHxDicItems/getItems

        // this.queryDic()
        this.queryDicItem(data)
      },
      queryDicItem (dicCode) {
        this.$http.get('/api/dic/cscpHxDicItems/getItems', {
          params: {
            'dicCode.equals': dicCode
          }
        }).then((response) => {
          let dicItemList = []
          dicItemList = response.data.data
          let resmap = []
          dicItemList.forEach((val) => {
            resmap.push({
              'label': val.itemValue,
              'value': val.itemCode
            })
          })
          this.optionModel.optionItems = resmap
