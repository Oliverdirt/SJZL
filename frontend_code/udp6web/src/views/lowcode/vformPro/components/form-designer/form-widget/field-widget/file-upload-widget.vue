    ...
    <el-upload ref="fieldEditor" :disabled="field.options.disabled || isReadMode"
      ...
      <template #file="{ file }">
        <div class="upload-file-list">
          ...
          <a :href="file.url" download="" target="_blank" rel="noopenner noreferrer">
...
import i18n, { translate } from '@/views/lowcode/utils/i18n'
import { deepClone } from '@/views/lowcode/utils/util'
...
export default {
  ...
  computed: {
    realUploadURL () {
      let uploadURL = this.$util.baseUrl + this.field.options.uploadURL
  ...
  methods: {
    ...
    beforeFileUpload (file) {
      ...
      // 添加用于测试/api/flow/bpmn/import接口所需要的bpmnFile参数,原组件只支持file参数
      this.uploadData.bpmnFile = file
    ...
    handleOnBeforeUpload (file) {
      if (!!this.field.options.onBeforeUpload) {
        let Fn = Function
        let bfFunc = new Fn('file', this.field.options.onBeforeUpload)
    ...
    handleFileUpload (res, file, fileList) {
      if (file.status === 'success') {
        ...
        if (!!this.field.options.onUploadSuccess) {
          let Fn = Function
          let mountFunc = new Fn('result', 'file', 'fileList', this.field.options.onUploadSuccess)
    ...
    removeUploadFile (fileName, fileUrl, fileUid) {
      ...
      if (foundIdx >= 0) {
        ...
        if (!!this.field.options.onFileRemove) {
          let Fn = Function
          let customFn = new Fn('file', 'fileList', this.field.options.onFileRemove)
    ...
    handleUploadError (err, file, fileList) {
      if (!!this.field.options.onUploadError) {
        let Fn = Function
        let customFn = new Fn('error', 'file', 'fileList', this.field.options.onUploadError)
