<template>
  ...
  <div class="ds-list">
    ...
    <el-drawer direction="rtl" :modal="true" :size="820" :show-close="false"
      ...
      <template #title>
        <div>{{i18nt('designer.setting.dataSourceSetting')}}</div>
        <div style="float: right">
          <el-button type="primary" plain @click="testDataSource">
            {{i18nt('designer.setting.testDataSource')}}</el-button>
          <el-button type="primary" @click="saveDataSource">
            {{i18nt('designer.setting.saveDataSource')}}</el-button>
      ...
      saveDataSource() {
        ...
        this.$refs['dsForm'].validate((valid, fields) => {
          ...
          if (this.curEditDSIdx >= 0) {
            this.formConfig.dataSources.splice(this.curEditDSIdx, 1, deepClone(this.dsModel))
