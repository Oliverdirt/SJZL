<template>
  ...
  <container-item-wrapper v-show="!widget.options.hidden" :widget="widget"
                          :style="{'margin-top': widget.options.marginTop + 'px'}">
    ...
    <el-table ref="dataTable" :data="widget.options.tableData" :class="[customClass]"
      ...
      <el-table-column v-if="widget.options.showCheckBox" type="selection"
                       :width="!widget.options.showSummary ? 50: 55" fixed="left"></el-table-column>
      ...
      <el-table-column v-for="(item,index) in widget.options.tableColumns"
        ...
        <template slot-scope="scope">
          <template v-if="item.formatS === 'render' && !!item.render">
            <table-column-custom-render :row="scope.row" :column="item" :render="getColumnRender(scope.row, item)"/>
          </template>
          ...
          <template #default="scope">
            <template v-for="(ob) in widget.options.operationButtons">
              <el-button v-show="showOperationButton(ob, scope.$index, scope.row)"
                         ...
                         @click="handleOperationButtonClick(ob, scope.$index, scope.row)"
    ...
    <el-pagination v-if="widget.options.showPagination"
                   ...
                   :style="{'justify-content': widget.options.pageWidth,'display': 'flex'}"
...
import {
  ...
  formatNumber6,
  formatNumber7
} from '@/views/lowcode/utils/format'
...
import { getDSByName, overwriteObj, runDataSourceRequest } from '@/views/lowcode/utils/util'
...
export default {
  ...
  methods: {
    ...
    getColumnRender (row, column) {
      ...
      let Fn = Function
      return new Fn('h', 'params', column.render)
    },
    ...
    handleOnCreated () {
      if (!!this.widget.options.onCreated) {
        let Fn = Function
        let customFunc = new Fn(this.widget.options.onCreated)
    ...
    handleOnMounted () {
      if (!!this.widget.options.onMounted) {
        let Fn = Function
        let customFunc = new Fn(this.widget.options.onMounted)
        customFunc.call(this)
      }
    },
    ...
    handleCurrentChange (currentRow, oldCurrentRow) {
      ...
      if (!!this.widget.options.onSelectionChange) {
        let Fn = Function
        let customFn = new Fn('selection', 'selectedIndices', this.widget.options.onSelectionChange)
    ...
    handlePageSizeChange (pageSize) {
      ...
      if (!!this.widget.options.onPageSizeChange) {
        let Fn = Function
        let customFn = new Fn('pageSize', 'currentPage', this.widget.options.onPageSizeChange)
    ...
    handleCurrentPageChange (currentPage) {
      ...
      if (!!this.widget.options.onCurrentPageChange) {
        let Fn = Function
        let customFn = new Fn('pageSize', 'currentPage', this.widget.options.onCurrentPageChange)
    ...
    handleOperationButtonClick (btnName, rowIndex, row) {
      let onOperationButtonClick = this.widget.options.onOperationButtonClick
      if (!!this.widget.options.onOperationButtonClick) {
        let Fn = Function
        let customFn = new Fn('buttonName', 'rowIndex', 'row', this.widget.options.onOperationButtonClick)
        customFn.call(this, btnName, rowIndex, row)
      } else {
        this.dispatch('VFormRender', 'operationButtonClick', [this, btnName, rowIndex, row, onOperationButtonClick])
      }
    },
    ...
    showOperationButton (buttonConfig, rowIndex, row) {
      if (!!this.widget.options.onHideOperationButton) {
        let Fn = Function
        let customFn = new Fn('buttonConfig', 'rowIndex', 'row', this.widget.options.onHideOperationButton)
    ...
    disableOperationButton (buttonConfig, rowIndex, row) {
      if (!!this.widget.options.onDisableOperationButton) {
        let Fn = Function
        let customFn = new Fn('buttonConfig', 'rowIndex', 'row', this.widget.options.onDisableOperationButton)
        return customFn.call(this, buttonConfig, rowIndex, row)
      } else {
    ...
    getOperationButtonLabel (buttonConfig, rowIndex, row) {
      if (!!this.widget.options.onGetOperationButtonLabel) {
        let Fn = Function
        let customFn = new Fn('buttonConfig', 'rowIndex', 'row', this.widget.options.onGetOperationButtonLabel)
        return customFn.call(this, buttonConfig, rowIndex, row)
      } else {
    ...
    getRowClassName ({ row, rowIndex }) {
      if (!!this.widget.options.onGetRowClassName) {
        let Fn = Function
        let customFn = new Fn('rowIndex', 'row', this.widget.options.onGetRowClassName)
    ...
    getSpanMethod ({ row, column, rowIndex, columnIndex }) {
      if (!!this.widget.options.onGetSpanMethod) {
        let Fn = Function
        let customFn = new Fn('row', 'column', 'rowIndex', 'columnIndex', this.widget.options.onGetSpanMethod)
    ...
    handleHeaderClick (column, event) {
      if (!!this.widget.options.onHeaderClick) {
        let Fn = Function
        let customFn = new Fn('column', 'event', this.widget.options.onHeaderClick)
    ...
    handleRowClick (row, column, event) {
      if (!!this.widget.options.onRowClick) {
        ...
        let Fn = Function
        let customFn = new Fn('row', 'column', 'event', this.widget.options.onRowClick)
    ...
    handleRowDoubleClick (row, column, event) {
      if (!!this.widget.options.onRowDoubleClick) {
        let Fn = Function
        let customFn = new Fn('row', 'column', 'event', this.widget.options.onRowDoubleClick)
        return customFn.call(this, row, column, event)
      }
    },

    handleCellClick (row, column, cell, event) {
      // 执行点击tablecell时将该tablecell的内容由text变为input
      // 找到对应的tablecell
      this.clickRowIndex = row.index
      this.tabClickLabel = column.label
      if (!!this.widget.options.onCellClick) {
        let Fn = Function
        let customFn = new Fn('row', 'column', 'cell', 'event', this.widget.options.onCellClick)
    ...
    handleCellDoubleClick (row, column, cell, event) {
      if (!!this.widget.options.onCellDoubleClick) {
        let Fn = Function
        let customFn = new Fn('row', 'column', 'cell', 'event', this.widget.options.onCellDoubleClick)
        return customFn.call(this, row, column, cell, event)
      }
    },
