...
<script>
  ...
  import i18n, {changeLocale} from "@/views/lowcode/utils/i18n"
    ...
    methods: {
      ...
      handleFieldDataChange(fieldName, newValue, oldValue, subFormName, subFormRowIndex) {
        if (!!this.formConfig && !!this.formConfig.onFormDataChange) {
          let Fn = Function
          let customFunc = new Fn('fieldName', 'newValue', 'oldValue', 'formModel', 'subFormName', 'subFormRowIndex', this.formConfig.onFormDataChange)
      ...
      handleOnCreated() {
        if (!!this.formConfig && !!this.formConfig.onFormCreated) {
          let Fn = Function
          let customFunc = new Fn(this.formConfig.onFormCreated)
      ...
      handleOnMounted() {
        if (!!this.formConfig && !!this.formConfig.onFormMounted) {
          let Fn = Function
          let customFunc = new Fn(this.formConfig.onFormMounted)
          customFunc.call(this)
        }
      },
