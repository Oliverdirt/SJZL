<template>
  <Modal v-model="configModal" title="配置页面" :closable="false" ref="modal" @on-cancel="close" :width="1568"
    ...
    <div slot="footer">
      <Button type="primary" v-if="step > 0" @click="back" style="width: 100px">
        <span>上一步</span>
      </Button>
      <Button type="primary" v-if="step < 3 && (step == 0 || formType != 3)" @click="next" style="width: 100px"
        :loading="nextLoading">
        <span v-if="!nextLoading">下一步</span>
        <span v-else>等待...</span>
      </Button>
      <Button type="primary" v-if="step == 3||(step != 0 && formType == 3)" @click="next" style="width: 100px"
        :loading="nextLoading">
        <span v-if="!nextLoading">完成</span>
        <span v-else>等待...</span>
      </Button>
...
export default {
  ...
  data() {
    return {
      fieldListApi: {
        URL: this.$util.baseUrl + '/api/cscpHxFormColums?tableName=' + this.tableName,
  ...
  watch: {
    tableName(newValue, oldValue) {
      ...
      this.fieldListApi.URL = this.$util.baseUrl + '/api/cscpHxFormColums?tableName=' + this.newValue
    },
    step(newV, oldV) {
      if (newV == 0) {
        this.$nextTick(() => {
  ...
  methods: {
    ...
    saveFormJson(nextStep) {
      ...
      this.$http.put('/api/lowcode/customize/cscpCustomizeVforms', this.detail).then(response => {
        ...
        if (this.formType == '2' || this.formType == '3') this.close()
        this.nextLoading = false
      }).catch(err => {
