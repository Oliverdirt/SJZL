<template>
  <div id="dataModelContainer" style="height: 100%">
    <div class="formCard-content">
          ...
          <Form ref="queryForm" :model="queryForm" :label-width="120">
            <Row :gutter="40">
              <Col span="8" style="margin-bottom: 12px">
                <FormItem label="数据模型名称" prop="modelName" style="width: 100%">
                  <Input type="text" :maxlength="100" v-model.trim="queryForm.modelName" placeholder="请输入数据模型名称"
                         style="width: 100%"/>
                </FormItem>
              </Col>
            ...
            <div class="tableBtn">
              <Button icon="md-add" type="primary" @click="choiseWayVisible=true" style="margin-right: 20px">
                新建数据模型
              </Button>
              <Button icon="ios-trash-outline" type="error" @click="handleDelete()" v-show="selectedArr.length>1">批量删除
              </Button>
            </div>
          </Form>
        </div>
      ...
      <Card :bordered="false" :dis-hover="true" :shadow="false" class="formCard">
        <Table border :columns="columns" :data="data" @on-selection-change="rowSelected($event)" style="">
          <template slot-scope="{ row }" slot="modelTypeAction">
            <span>{{ row.modelType === '1' ? '单表模型' : row.modelType === '3' ? '视图模型' : '主子表模型' }}</span>
          </template>
          <template slot-scope="{ row }" slot="createModeAction">
            <span>{{ row.createMode === '1' ? '从数据库中新建' : '新建' }}</span>
          </template>
          <template slot-scope="{ row, index }" slot="action">
            <Button v-if="row.createMode == '1'" type="text" size="small" style="margin-right: 5px; color: #246AD9"
                    @click="okFlag = 'update';viewTable(row, index);
                     ">查看
            </Button>
            <Button v-else type="text" size="small" style="margin-right: 5px; color: #246AD9" @click="okFlag = 'update';editTable(row, index);
                    ">编辑
            </Button>
            <Button type="text" size="small" style="margin-right: 5px; color: #F53F3F" @click="remove(row, index)">
              删除
            </Button>
          </template>
        </Table>
    ...
    <el-dialog title="编辑" :visible.sync="editTableVisible" width="90%">
      <el-table :data="tableData" row-key="key" style="overflow-y:scroll" :row-style="{ height: '70px' }"
        ...
        <el-table-column label="是否为空" prop="isNull" style="width:auto">
          <template v-slot="scope">
            ...
            <!--            </RadioGroup>-->
            <el-radio-group size="mini" v-model="scope.row.isNull" :disabled="scope.row.editFlag">
              <el-radio-button :disabled="scope.row.editFlag" label="1">是</el-radio-button>
              <el-radio-button :disabled="scope.row.editFlag" label="0">否</el-radio-button>
            </el-radio-group>
          </template>
        </el-table-column>
        <el-table-column label="是否为主键" prop="isPk" style="width:auto">
          <template v-slot="scope">
            ...
            <!--            </RadioGroup>-->
            <el-radio-group size="mini" v-model="scope.row.isPk" :disabled="scope.row.editFlag">
              <el-radio-button :disabled="true" label="1">是</el-radio-button>
              <el-radio-button :disabled="true" label="0">否</el-radio-button>
            </el-radio-group>
        ...
        <el-table-column label="是否为业务主键" prop="isBk" style="width:auto">
          <template v-slot="scope">
            ...
            <!--              <Radio label="0" :disabled="scope.row.editFlag">否</Radio>-->
            <!--            </RadioGroup>-->
            <el-radio-group size="mini" v-model="scope.row.isBk" :disabled="scope.row.editFlag">
              <el-radio-button :disabled="scope.row.editFlag" label="1">是</el-radio-button>
              <el-radio-button :disabled="scope.row.editFlag" label="0">否</el-radio-button>
            </el-radio-group>
          </template>
        ...
        <el-table-column label="操作" style="width:auto">
          <template v-slot="scope">
            ...
            <!--            <el-button v-if="tableData.length >= 1" type="text" size="mini" @click="peerTable(scope.row)">新建</el-button>-->
          </template>
        </el-table-column>
      </el-table>
      <div class="btnBox">
        <el-button v-if="selectedRowData.length === 1 && selectedRowData[0].childDto"
    ...
    <!--    新建数据模型 -->
    <CreateModel :moduleParam="moduleParam" :moduleNameList="moduleNameList"
                 :createModelDialogVisible="createModelDialogVisible"></CreateModel>
    <!--    从数据库中创建数据模型弹窗 -->
    <addModelFromDb :moduleParam="moduleParam" @childCancel="childCancel" v-if="createModelDbVisible"
                    ref="addModelFromDb"></addModelFromDb>
    <!--    选择创建数据模型方式 -->
    <div class="cmodelWay">
      <el-dialog
        title="新建数据模型"
        :visible.sync="choiseWayVisible"
        width="30%"
      >
        <div class="dBody">
          <div class="label"><span style="color:red">*</span> 创建方式：</div>
          <el-radio-group v-model="modelWayVal">
            <el-radio :label="0">已有表创建</el-radio>
            <el-radio :label="1">新建表创建</el-radio>
          </el-radio-group>
        </div>
        <span slot="footer" class="dialog-footer">
          <el-button type="primary" plain @click="choiseWayVisible = false">取 消</el-button>
          <el-button type="primary"
                     @click="choiseWayVisible = false;modelWayVal === 0?clickDbBtn():openAddModelDialog()">确 定</el-button>
      </span>
      </el-dialog>
    </div>
  </div>
...
export default {
  ...
  name: 'dataModelMng',
  components: {CreateModel, addModelFromDb},
  data() {
  ...
  created() {
    this.moduleParam = this.$route.query
  },
  mounted() {
  ...
  methods: {
    ...
    remove(row, index) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.$http
            .delete('api/cscpCustomizeModels/' + row.modelId)
            .then((res) => {
              if (res.data.code === 500) {
                this.$Message.error('此数据模型包含已创建的页面,请先删除页面再删除此数据模型!')
                return
              }
              if (res.data.msg) {
    ...
    queryList(flag) {
      ...
      const moduleId = this.moduleParam.moduleId || ''
      const modelName = this.queryForm.modelName || ''
      let url = `api/cscpCustomizeModels?moduleId.equals=${moduleId}&modelName.contains=${modelName}&page=${page}&size=${size}&sort=create_time,desc`
    ...
    handleDelete() {
      let arr = this.selectedArr.map((item) => item.modelId)
      if (arr.length < 1) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.$http
            .delete('api/cscpCustomizeModels/delAll?modelIds=' + arr)
            .then((res) => {
              if (res.data.code === 500) {
                this.$Message.error('此数据模型包含已创建的页面,请先删除页面再删除此数据模型!')
                return
    ...
    getModuleNameList() {
      ...
      this.$http
        .get(
          `api/cscpCustomizeModules?page=${page}&size=${size}`
        )
        .then((res) => {
          ...
          for (let index = 0; index < tempArr.length; index++) {
            const obj = {}
            obj.label = tempArr[index].moduleName
            obj.value = tempArr[index].moduleId
            this.moduleNameList.push(obj)
          }
        })
        .catch(() => {
          this.$Message.error('列表查询失败')
