...
export function getDefaultFormConfig(onFormMounted = '') {
  return {
    modelName: 'formData',
    refName: 'vForm',
    rulesName: 'rules',
...
export function assembleAxiosConfig(arrayObj, DSV, VFR) {
  ...
  arrayObj.map(ai => {
    if (ai.type === 'String') {
      ...
      let Fn = Function
      result[ai.name] = new Fn(ai.value)
    }
  })
...
function buildRequestConfig(dataSource, DSV, VFR, isSandbox) {
  ...
  if (dataSource.requestURLType === 'String') {
    ...
    let Fn = Function
    config.url = new Fn(dataSource.requestURL)
  }
  ...
  let Fn = Function
  let chFn = new Fn('config', 'isSandbox', 'DSV', 'VFR', dataSource.configHandlerCode)
...
export async function runDataSourceRequest(dataSource, DSV, VFR, isSandbox, $message) {
  try {
    ...
    let Fn = Function
    let dhFn = new Fn('result', 'isSandbox', 'DSV', 'VFR', dataSource.dataHandlerCode)
    return dhFn.call(null, result, isSandbox, DSV, VFR)
  } catch (err) {
    let Fn = Function
    let ehFn = new Fn('error', 'isSandbox', 'DSV', '$message', 'VFR', dataSource.errorHandlerCode)
