import {isNotNull} from "@/views/lowcode/utils/util";
import {genVue2JS} from "@/views/lowcode/utils/vue2js-generator";
import {beautifierOpts} from "@/views/lowcode/utils/beautifierLoader";
import {genVue3JS} from "@/views/lowcode/utils/vue3js-generator";
...
const elTemplates = {  //字段组件属性
  ...
  //高级组件
  //1.部门组件
  'dept': (widget, formConfig) => {
    const {vModel, readonly, disabled, size, type, showPassword, placeholder, clearable,
      minlength, maxlength, showWordLimit, prefixIcon, suffixIcon, appendButtonChild} = getElAttrs(widget, formConfig)
    return `<Treeselect  ${vModel}  ${readonly} ${disabled} ${size} ${placeholder} ${clearable}
            ${minlength} ${maxlength} ${showWordLimit} ${prefixIcon} ${suffixIcon}  :options="deptArr"  placeholder="请输入部门"  autocomplete=“off” />`
  },
  //2.高级组件
