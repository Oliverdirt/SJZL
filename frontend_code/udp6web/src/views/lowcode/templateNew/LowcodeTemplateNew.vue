...
import ChildTableInfo from '@/views/lowcode/templateNew/ChildTableInfoNew'
  ...
  methods: {
    ...
    initPage(to) {
      ...
      if (window.location.href.includes('customize/page/')) {
        if (window.location.href.split('customize/page/')[1]) {
          // 拿到pageId
          this.pageId = window.location.href.split('customize/page/')[1]
    ...
    edit(row, buttonName) {
      if (buttonName.opeType === '编辑') {
        ...
        this.cPageId = buttonName.pageId
        this.relationField = buttonName.relationField
      }
    },
    ...
    fileChange(el) {
      ...
      this.$axios({
        method: 'post',
        url: util.baseUrl + urlApi + this.pageId + '?pageId=' + this.pageId + '&queryStr=' + encodeURIComponent(JSON.stringify(temJson)), // 请求接口
