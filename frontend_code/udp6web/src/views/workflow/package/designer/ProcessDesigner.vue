<template>
  <div class="my-process-designer">
    <div class="my-process-designer__header">
      ...
      <template v-if="!$slots['control-header']">
        <el-button-group key="file-control">
          <el-button :size="headerButtonSize" type="" icon="el-icon-folder-opened" @click="$refs.refFile.click()"></el-button>
          <el-tooltip effect="light">
            ...
            <el-button :size="headerButtonSize" type="" icon="el-icon-download"></el-button>
          </el-tooltip>
          <el-tooltip effect="light">
            <div slot="content">
            ...
            <el-button :size="headerButtonSize" type="" icon="el-icon-view"></el-button>
          </el-tooltip>
          <el-tooltip v-if="simulation" effect="light" :content="this.simulationStatus ? '退出模拟' : '开启模拟'">
            <el-button :size="headerButtonSize" type="" icon="el-icon-cpu" @click="processSimulation">
            </el-button>
          </el-tooltip>
        </el-button-group>
        ...
        <el-button size="mini" v-show="!category" style="margin-left: 5px;padding: 5.2px 8px;" :type="headerButtonType" icon="el-icon-document-checked" @click="processParams">流程参数</el-button>
        <el-button size="mini" style="margin-left: 5px;padding: 5.2px 8px;" :type="headerButtonType" icon="el-icon-document-checked" @click="processSave">保存模型</el-button>
      </template>
      <!-- 用于打开本地文件-->
    ...
    <!--表单弹框-->
    <el-dialog title="流程参数配置" :visible.sync="processOpen" width="600px" append-to-body >
        <el-form ref="formData" :model="form" label-width="80px">
          <el-form-item label="流程参数">
                <CheckboxGroup v-model="formParams" @on-change="formParamsFn">
                    <Checkbox v-for="item in formParamsOptions" :label="item.value" :key="item.value + 'ss'"></Checkbox>
                </CheckboxGroup>
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
          <el-button @click="formParamsSave" type="primary">确 定</el-button>
...
export default {
  ...
  methods: {
    // 配置流程参数
    modelEditorDataFn(row,category){
      console.log(row,category,'row1');
    ...
    processParams(){
      ...
      this.params = {
          taskDefKey: this.modelEditorData.key,
          formId: this.modelEditorData.formId
        }
      this.$http.post('/api/cscpFlowFieldss/findOne', this.params).then((res) => {
        console.log(res,'sd');
        this.formParamsArr = res.data.data
        this.formParamsOptions = []
        for (const item of this.formParamsArr) {
          this.formParamsOptions.push({
            value: item.fieldName,
            label: item.fieldName
          })
        }
        this.formParams = res.data.data.filter((item) => item.isFlowField === '1').map(item => item.fieldName)
      })
    },
    formParamsFn (e) {
      this.arrayData = this.formParamsArr.map((item) => {
        return {
          fieldName: item.fieldName,
          isFlowField: e.includes(item.fieldName) ? '1' : '0'
        }
      })
    },
    // 保存参数
    formParamsSave () {
      let params = {
        taskDefKey: this.modelEditorData.key,
        formName: this.modelEditorData.formName,
        formId: this.modelEditorData.formId,
        fieldsPerms: this.arrayData ? this.arrayData : this.formParamsArr
      }
      this.$http.post('/api/cscpFlowFieldss/insertOne', params).then((res) => {
