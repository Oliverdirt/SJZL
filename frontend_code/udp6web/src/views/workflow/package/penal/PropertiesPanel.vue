<template>
  <div class="process-panel__container" :style="{ width: `${this.width}px` }" ref="MyPropertiesPanel">
    <div style="position:absolute;margin-left: -13px;top: 50%;font-size: 17px;cursor: pointer;">
      <i v-if = tabVisible @click="addFn" class="el-icon-arrow-right"></i>
      <i v-else @click="delFn" class="el-icon-arrow-left"></i>
    </div>
...
<!--        <element-form :id="elementId" :type="elementType" />-->
        友情提示：使用 <router-link target="_blank" :to="{path:'/lowcode-programming'}"><el-link type="danger">表单设计</el-link> </router-link>
        替代，提供更好的表单设计功能
      </el-collapse-item>
      <el-collapse-item name="task" v-if="elementType.indexOf('Task') !== -1 || elementType.indexOf('Activity') !== -1" key="task">
      ...
      <el-collapse-item name="multiInstance" v-if="elementType.indexOf('Task') !== -1" key="multiInstance">
        ...
        :loopInstanceNameList="loopInstanceNameList"
        :type="elementType" />
      </el-collapse-item>
      <el-collapse-item v-if="elementType === 'SubProcess'">
        <div slot="title" class="panel-tab__title"><i class="el-icon-s-help"></i>多实例子流程</div>
        <element-multi-instance :business-object="elementBusinessObject"
        :loopInstanceNameList="loopInstanceNameList"
        :type="elementType" />
      </el-collapse-item>
      ...
      <el-collapse-item name="extensions" key="extensions">
        ...
        <element-properties :id="elementId" :type="elementType" ref="elementProperties" />
      </el-collapse-item>
...
export default {
  ...
  methods: {
    ...
    getLoopInstanceNameList() {
      this.$http.get('/api/flow/multi/listAllMultiStrategy').then(res => {
        ...
        this.loopInstanceNameList = temp
      })
    },
    // 初始化数据
