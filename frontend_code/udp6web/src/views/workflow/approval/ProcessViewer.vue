<template>
  <div class="my-process-designer">
    <div class="my-process-designer__container" style="z-index: 100;">
      <el-container style="height: 100%;overflow:hidden;z-index: 100;">
        <el-header style="border-bottom: 1px solid rgb(218 218 218);height: auto;z-index: 100;margin: 0px;">
          <div style="display: flex; padding: 0px 0px 10px 0px; justify-content: center;">
            <div>
        ...
        <div id="boxFindWindow">
          <div class="my-process-designer__canvas" ref="bpmn-canvas" id="findwindow"></div>
        </div>
      </el-container>
    </div>
  </div>
...
import { dragBox } from './plugins/ProcessViewerDrag.js'
...
export default {
  ...
  mounted () {
    this.$nextTick(() => {
      ...
      dragBox(document.querySelector('#findwindow .djs-container svg>g'), document.querySelector('#findwindow .djs-container svg'))
    }),
      this.xml = this.value
  ...
  methods: {
    ...
    async highlightDiagram () {
      let flowElements = this.bpmnModeler.getDefinitions().rootElements[0].flowElements
      this.testForEach(flowElements)
    },
    testForEach (flowElements) {
      const activityList = this.activityList
      if (activityList.length === 0) {
        return
      }
      console.log("1:",activityList)
      console.log("2:",flowElements)
      ...
      flowElements?.forEach(n => {
        ...
        if (n.$type === 'bpmn:UserTask') { // 用户任务
          ...
          // 处理 outgoing 出线
          const outgoing = this.getActivityOutgoing(activity, flowElements)
          outgoing?.forEach(nn => {
            // debugger
          ...
          canvas.addMarker(n.id, this.getResultCss(activity.endTime ? 2 : 1))
          if (activity.endTime) {
            n.outgoing?.forEach(nn => {
              let targetActivity = activityList.find(m => m.key === nn.targetRef.id)
              canvas.addMarker(nn.id, this.getResultCss(2))
            })
          }
        ...
        } else if (n.$type === 'bpmn:SubProcess') { //子流程

          let endEventArray = activityList.filter((item) => item.type === 'endEvent').map((itemC)=>{
            // 结束节点亮
            canvas.addMarker(itemC.key, this.getResultCss(2))
            return itemC.key
          })
          let EndEventId = n.flowElements.filter((item) => item.$type == 'bpmn:EndEvent')[0].id
          if(endEventArray.indexOf(EndEventId) !== -1){
            canvas.addMarker(n.id, this.getResultCss(2))
            canvas.addMarker(n.outgoing[0].id, 'highlight')
          }
          this.testForEach(n.flowElements)
        }
      })
    },
    ...
    getActivityOutgoing (activity, childFlowElemnts) {
      // 如果有 outgoing，则直接使用它
      ...
      const flowElements = childFlowElemnts
      const outgoing = []
    ...
    elementHover (element) {
      ...
      if (!this.elementOverlayIds[element.id] && element.type !== 'bpmn:Process') {
        ...
        if (element.type === 'bpmn:StartEvent' && this.processInstance) {
          html = `<p>发起人：${this.processInstance.startName}</p>`
        } else if (element.type === 'bpmn:UserTask') {
          // debugger
          let ids = []
          this.activityList.filter(m => m.key === element.id).forEach(e => {
            ids.push(e.taskId)
          })
          if (ids.length === 0) {
            return
          }
          let taskList = this.taskList.filter(m => ids.indexOf(m.id) !== -1) // 找到活动对应的 taskId
          if (taskList.length === 0) {
            return
          }
          taskList.forEach(e => {
            html += `<p>审批人：${e.assigneeUser.nickname}</p>`
            if (e.reason) {
              html += `<p>处理意见：${e.reason}</p>`
            }
          })
        }
        if (html != '') {
