<template>
  <div style="height: 100%">
    <div class="formCard-content">
      ...
      <Card :bordered="false" :dis-hover="true" :shadow="false" class="formCard">
        <div class="tableBtn">
          ...
          <Button  icon="md-sync"  type="warning"  @click="handleResetDictCache()" style="margin-left: 10px">刷新缓存</Button>
          <Button  icon="ios-trash-outline" type="error"  @click="handleDeleteMultiple()" style="margin-left: 10px">删除</Button>
        </div>
        <Table border :columns="columns" :data="data" @on-selection-change="rowSelected($event)" style="">
          <template slot-scope="{ row, index }" slot="action">
            <Button type="text" size="small"  style="margin-right: 5px;color: #0056B5" @click="editDicItem(row.dicId,row.dicCode)">字典配置</Button>
    ...
    <dic-item-list ref="dicItemList"></dic-item-list>
  </div>
</template>
...
export default {
  name: 'dicManager',
  components: { DicItemList },
  data () {
    // 自定义校验
    const validateDicCode = (rule, value, callback) => {
      this.dicDetail.dicAttr = 'list'
      // 检查dicCode是否存在
  ...
  methods: {
    ...
    // 编辑字典数据
    editDicItem (dicId, dicCode) {
      this.$refs.dicItemList.edit(dicId, dicCode)
    },
    handleAdd () {
      this.dicDetail = {}
      this.okFlag = 'add'
          ...
          delDicListById(row.dicId)
            .then(res => {
              if (res.data.code !== 200) {
                ...
                this.$store.dispatch('dict/removeDict', row.dicCode)
              }
              this.queryList()
            }).catch(() => {
        ...
        if (valid) {
          if (type === 'update') {
            updateDicListByData(copyDetail)
              .then(res => {
                if (res.data.code === 200) {
                  this.$store.dispatch('dict/removeDict', this.dicDetail.dicCode)
                  this.$Message.success(res.data.msg)
                } else {
                  this.$Message.error(res.data.msg)
                }
            ...
            .then(res => {
              if (res.data.code === 200) {
                ...
                this.selectedArr.forEach(item => {
                  this.$store.dispatch('dict/removeDict', item.dicCode)
                })
                this.queryList()
