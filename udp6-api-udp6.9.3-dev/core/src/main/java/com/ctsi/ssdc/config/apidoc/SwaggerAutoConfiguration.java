...
import com.ctsi.ssdc.config.apidoc.customizer.CtsiSwaggerCustomizer;
import com.ctsi.ssdc.config.apidoc.customizer.SwaggerCustomizer;
...
@Profile("swagger")
@AutoConfigureAfter(CtsiProperties.class)
@EnableSwagger2
...
public class SwaggerAutoConfiguration {
    ...
    public SwaggerAutoConfiguration(CtsiProperties ctsiProperties) {
        this.properties = ctsiProperties.getSwagger();
    }
     ...
     */
    @Bean
    public CtsiSwaggerCustomizer ctsiSwaggerCustomizer() {
        return new CtsiSwaggerCustomizer(properties);
    }
    ...
    public Docket swaggerSpringfoxManagementDocket(@Value("${spring.application.name:application}") String appName,
        ...
        return createDocket()
            .apiInfo(apiInfo)
            .securitySchemes(Arrays.asList(new ApiKey("Authorization","Authorization","Authorization"))) 
