...
public class HttpUtil {
    ...
    /**
     * 向指定URL发送GET方法的请求,可携带请求头
     */
    public static String sendGet(String url, String param, Map<String, String> header, Integer timeOut, Integer readTimeout) throws UnsupportedEncodingException, IOException {
        String result = "";
    ...
    public static String getString(String httpUrl) {
        ...
        postMethod = new HttpGet(httpUrl);
     ...
     * @return 响应消息
     */
    public static String post1(String url, Map<String, String> params, String charset, HttpClient httpclient) throws Exception {
        boolean isDefaultHc = false;
        if (url == null || "".equals(url)) {
            return null;
        }
        ...
        try {
            HttpResponse execute = httpclient.execute(hp);
            Header firstHeader = execute.getFirstHeader("Set-Cookie");
            responseStr = firstHeader.getValue();
        } catch (ClientProtocolException e) {
            throw new Exception("客户端连接协议错误", e);
        } catch (IOException e) {
            throw new Exception("IO操作异常", e);
